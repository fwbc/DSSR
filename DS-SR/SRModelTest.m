...
%{
f = @(t, y) [  -35.0 * y(1) + 35.0 * y(2)  ;
            28.00019 * y(2) + (-y(1) - 1.801629e-6) .* (y(3) + 7.0002847) ; 
           y(1) .* y(2) - 2.9999678 * y(3)];
%}

f = @(t, x) [ -35.0*x(1) + 35.0*x(2) ;
           x(1) + 28.0*x(2) + (1.063722e-7 - x(1))*(x(3) + 8.000001) ; 
           x(1)*x(2) - 3.0*x(3)];
